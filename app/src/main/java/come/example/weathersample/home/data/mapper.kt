...
import come.example.weathersample.home.domain.model.City
...
fun WeatherEntity.toModel(): Weather = Weather(city?.map { it -> it.toModel() }, lat, lng, current?.toModel())
...
fun Weather.toEntity() = WeatherEntity(city?.map { it -> it.toEntity() }, lat, lng, current?.toEntity())
